  ...
  return (
    <>
      ...
      {!hidden && (
        <div
          ref={petRef}
          className={`${css.pet} ${isDragging ? css.dragging : ''}`}
          style={pos === null
            ? undefined
            : { left: `${pos.x}px`, top: `${pos.y}px`, right: 'auto', bottom: 'auto' }}
        >
          <div className={css.fishAnchor}>
            {showBubble && (
              <div className={css.bubble}>
                <div className={css.bubbleMood}>{moodInfo.tag}</div>
                <div className={css.bubbleText}>{speech || '🐟 咸鱼发呆中…'}</div>
              </div>
            )}
            <div className={css.hungerBar} title={`饱腹度 ${hunger}/100`}>
              <div className={css.hungerFill} style={{ width: `${hunger}%` }} />
            </div>
            <div
              className={`${css.fishWrap} ${showBubble ? css.active : ''}`}
              ...
              title="点我喂食 / 拖我移动"
            >
              <span className={css.fish}>{moodInfo.emoji}</span>
